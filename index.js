  ...
  contentFor: function(type, config) {
    ...
    if (apiKey && type === 'body-footer') {
      return '<script type="text/javascript">' +
        ...
        ' g.async = true;g.src = \'https://embed.doorbell.io/button/3744?t=\'+(new Date().getTime());' +
        ' (d.getElementsByTagName(\'head\')[0]||d.getElementsByTagName(\'body\')[0]).appendChild(g); }' +
        ' if (w.attachEvent) { w.attachEvent(\'onload\', l); } else if (w.addEventListener) { ' +
        ' w.addEventListener(\'load\', l, false); } else { l(); }' +
